...
{
    /// Coroutine to resume when this task completes (the awaiting coroutine).
...
class Task
{
  ...
  public:
      ...
      public:
        ...
        decltype(auto) await_resume()
        {
            if (!handle_)
            {
                ...
                {
                    throw std::logic_error("co::Task: awaiting an empty task");
                }
            }

            std::fprintf(stderr, "[co::Task] completed\n");
     ...
     *
     * The rvalue-only overload moves the handle out of the task, enforcing the
     * single-consumer rule: after co_await the task is empty and cannot be
     * awaited again.
     *
