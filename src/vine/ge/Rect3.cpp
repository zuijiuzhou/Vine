...
TMPL_PREFIX Rect3<T>::Rect3(const Point3<T>& bottom_left_front, const Vector3<T>& size)
  : x(bottom_left_front.x)
  , y(bottom_left_front.y)
  , z(bottom_left_front.z)
  , l(size.x)
  , w(size.y)
  , h(size.z) {
}
...
TMPL_PREFIX T Rect3<T>::lenght() const {
    return l < 0 ? -l : l;
}
