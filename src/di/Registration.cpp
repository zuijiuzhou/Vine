...
Registration::Registration(Type type) : d(new Data()) {
  d->service_type = type;
}

Registration::Registration(const Registration &reg) : d(new Data()) {
  *d = *reg.d;
}
...
Lifetime Registration::lifetime() const { return d->lifetime; }
