 ...
 * \brief 停靠系统的核心管理器。
 *
 * 持有整棵停靠树（根节点 m_rootPane）、中央客户区（DockingPaneClient）、
 * 四个自动隐藏按钮条、拖放指示器（DockingFrameStickers / DockingTargetWidget）
 * 以及当前 flyout。负责窗格的创建、停靠、关闭、隐藏/显示、浮动、自动隐藏，
 * 以及布局的保存与恢复。宿主通过 widget() 取得顶层控件嵌入主窗口。
 *
 * \note 注意事项：
 *  - widget() 未调用前四个自动隐藏条未创建，此时调用 saveLayout() /
 *    updateAutohideButton() / removePinnedButton() 会解引用空指针。
 *  - closePane() 对“已提交的 Tab 子窗格”（state()==Tabbed）会路由到所属
 *    DockingPaneTabbedContainer::closePane() 完成标签移除；applyLayout() 恢复
 *    期间（m_closingAll）不路由，避免触发 close 回调与过早拆组。
 *  - createPane() 使用重复 id 会直接覆盖 m_dockingPaneMap 中的旧条目。
 *  - 本类不删除停靠树（m_dockingWidget / m_thisWidget / m_clientPane），
 *    它们归宿主（如 setCentralWidget）所有，析构时仅清理内部指示器与 flyout。
 */
