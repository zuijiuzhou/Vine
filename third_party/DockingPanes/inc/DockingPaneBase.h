 ...
 * \brief 停靠系统所有节点的抽象基类。
 *
 * 停靠树中的每个元素（单窗格 DockingPaneContainer、标签组
 * DockingPaneTabbedContainer、分割器 DockingPaneSplitterContainer、
 * 中央客户区 DockingPaneClient）都继承自本类。提供统一的身份
 * （id/name）、状态（State）与宿主数据（userData）。
 *
...
class DockingPaneBase : public QWidget
{
    ...
    protected:
         ...
         * \note 仅供派生类与 DockingPaneManager（friend）使用，宿主请通过
         * 管理器的高层操作（dockPane/closePane/hidePane 等）改变状态。
         */
