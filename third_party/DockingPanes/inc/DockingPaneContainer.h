 ...
 *
 * 是停靠树中的基本叶子节点。可处于停靠、浮动、自动隐藏、Tab 四种形态
 * （见 DockingPaneBase::State）。浮动时为 Qt::ToolTip 无边框置顶窗口，
 * 带边缘缩放光晕（DockingPaneGlow）。
 *
 * \note 注意事项：
 *  - 默认构造 DockingPaneContainer(QWidget*) 仅面向派生类
 *    （DockingPaneTabbedContainer）使用，成员未初始化，直接调用其方法会崩溃。
 *  - 作为 Tab 子窗格时，只有 clientWidget 被放进标签组，容器对象本身不
 *    reparent；关闭/移动必须经由 DockingPaneManager::closePane() 路由或
 *    DockingPaneTabbedContainer::closePane()，不要直接操作。
 *  - m_flyoutWidget 为 QPointer，flyout 被管理器删除后自动置空。
 *  - 拖动标题栏超过 5px 会转为浮动（floatPane），期间窗口重建会丢失鼠标
 *    抓取，需要 reacquireGrab()/takeGrab() 重新抓取。
 */
