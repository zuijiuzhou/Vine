 ...
 *
 * 内部用 QStackedWidget 承载各子窗格的 clientWidget，底部手绘标签条
 * （DockingPaneTabbedContainer 重写了 paintEvent 与鼠标事件）。当标签数
 * 降到 1 时自动“折叠”回单窗格并删除自身。
 *
 * \note 关键设计：子窗格只把 clientWidget 放入 QStackedWidget，子容器对象
 * 本身不被 reparent 进本容器。因此：
 *  - 关闭/移动子窗格必须调用本类 closePane() 或
 *    DockingPaneManager::closePane()（会路由到这里），直接删子容器会使
 *    本容器的 m_paneList 悬垂。
